...
class TestTokenBackend(TestCase):
    ...
    def test_decode_hmac_with_no_expiry_no_verify(self):
        ...
        self.assertEqual(
            self.hmac_token_backend.decode(no_exp_token, {"verify_signature": False}),
            self.payload,
        )
    ...
    def test_decode_hmac_with_invalid_sig_no_verify(self):
        exp = aware_utcnow() + timedelta(days=1)
        self.payload["exp"] = timegm(exp.utctimetuple())
        token_1 = jwt.encode(self.payload, SECRET, algorithm="HS256")
        self.payload["foo"] = "baz"
        token_2 = jwt.encode(self.payload, SECRET, algorithm="HS256")
        ...
        self.assertEqual(
            self.hmac_token_backend.decode(invalid_token, {"verify_signature": False}),
            self.payload,
        )
    ...
    def test_decode_rsa_with_expiry(self):
        exp = aware_utcnow() - timedelta(seconds=1)
        self.payload["exp"] = timegm(exp.utctimetuple())
